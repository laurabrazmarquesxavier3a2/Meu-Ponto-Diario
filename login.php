    ...
    if ($resultado->num_rows > 0) {
        ...
        if (
            ...
            $minutosRestantes = ceil(
                (strtotime($usuario['bloqueado_ate']) - time()) / 60
            );
        ...
        // SE NÃO ESTIVER BLOQUEADO
            ...
            if ($tentativas >= 3) {
                ...
                $erro = "Conta bloqueada por 5 minutos.";
            } else {

                $stmtTentativas = $con->prepare(
                    "UPDATE usuarios
                 SET tentativas_login = ?
                 WHERE id_usuario = ?"
                );

                $stmtTentativas->bind_param(
                    "ii",
                    $tentativas,
                    $usuario['id_usuario']
                );
                ...
                if ($tentativas == 2) {
                    ...
                    $erro = "E-mail ou senha inválidos. Atenção: esta é sua última tentativa antes do bloqueio.";
                } else {
